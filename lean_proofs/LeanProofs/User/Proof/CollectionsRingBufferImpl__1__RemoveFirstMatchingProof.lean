...
def rfm_k0 : Int → Slc Int → Int → Int → Int → Prop :=
  fun idx ring _ _ _ =>
    0 ≤ idx ∧ idx < collections_sslice_len ring

def rfm_k3 : Int → Int → Slc Int → Slc Int → Int → Int → Int → Int → Int → Prop → Int → Int → Prop :=
...
def rfm_k4 :  Int → Int → Int → Slc Int → Slc Int → Int → Int → Int → Int → Int → Prop → Int → Int → Prop :=
...
def CollectionsRingBufferImpl__1__RemoveFirstMatching_proof : CollectionsRingBufferImpl__1__RemoveFirstMatching := by
  ...
  exists rfm_k0 ; exists k1 ; exists k2 ; exists rfm_k3
  ...
  any_goals (grind only [rfm_k0, k1, rfm_k3, k3, k4, k5, k6, k7])
  any_goals (simp_all [rfm_k3])
